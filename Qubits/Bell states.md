...
operation BellPhi_Minus(q0 : Qubit, q1 : Qubit) : Unit is Adj {
    H(q0);
    CNOT(q0, q1);
...
operation BellPsi_Plus(q0 : Qubit, q1 : Qubit) : Unit is Adj {
    H(q0);
    CNOT(q0, q1);
...
operation BellPsi_Minus(q0 : Qubit, q1 : Qubit) : Unit is Adj {
    H(q0);
    CNOT(q0, q1);
    X(q1);
    Z(q0);
...
**GHZ state** (multi-qubit entanglement). Generalizes $|\Phi^+\rangle$ to $n$ [[Qubits]]: $|GHZ_n\rangle = \frac{1}{\sqrt{2}}(|0\rangle^{\otimes n} + |1\rangle^{\otimes n})$
```csharp
