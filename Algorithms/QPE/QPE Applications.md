#Algorithm #Math
**Phase estimation** is not standalone algorithm - it is the core subroutine behind most exponential quantum speedups. Any problem that reduces to estimating the eigenvalue $e^{2\pi i\varphi}$ of unitary $U$ can be solved by [[QPE]].
...
| [[QPE Chemistry\|Quantum chemistry]] | Time evolution $e^{-iHt}$ | Ground state energy $E_0$ |
...
| [[QPE Walks\|Quantum walks]] | Walk operator $W$ | Spectral gap / mixing time |
...
$$t = \left\lceil \log_2 \frac{1}{\epsilon} + \log_2\!\left(2 + \frac{1}{2\delta}\right)\right\rceil \text{ clock [[[[Qubits]]]]}$$
