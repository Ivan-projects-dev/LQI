#Algorithm 
**Graph coloring problem**: given undirected graph $G = (V, E)$ with $|V| = n$ vertices & palette of $K$ colors, find **proper coloring** - an assignment $c: V \to \{0,\ldots,K-1\}$ such that no $2$ adjacent vertices share color:
$$\forall (u,v) \in E : c(u) \neq c(v)$$
