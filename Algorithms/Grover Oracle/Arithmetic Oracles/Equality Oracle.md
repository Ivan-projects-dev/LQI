...
`ControlledOnInt(n, op)(controls, target)` applies `op` to `target` iff the integer encoded in `controls` (little-endian) equals `n`. Internally it applies $X$ to every control qubit where corresponding bit of `n` is $0$, does multi-controlled $X$, then uncomputes - exactly `ArbitraryPattern` trick in [[Grover oracles]]. For fixed-length bit pattern instead of int:
```csharp
import Std.Canon.*;
