...
Phase kickback from $|-\rangle$ imprints $(-1)^{s \cdot x}$ on $|x\rangle$. State after the [[Oracle]] is $H^{\otimes n}|s\rangle$. $2nd$ Hadamard layer yields $|s\rangle$ deterministically - one shot recovers all $n$ bits of $s$.
```csharp
import Std.Arrays.*;
import Std.Convert.*;
