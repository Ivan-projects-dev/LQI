...
Classical computers cannot factor large nums efficiently. The best known classical algorithm (General num Field Sieve) runs in sub-exponential time $O(e^{n^{1/3}})$ for an $n$-bit num. Shor's algorithm runs in polynomial time $O(n^3)$ - an exponential speedup.
## Sources
