...
Applied to $|0\rangle$ it creates **uniform superposition**: both outcomes $0$ and $1$ are equally likely on measurement. Applied to $|1\rangle$ it creates the same uniform superposition but with a **relative phase flip** between the two terms - the phase encodes classical info that a quantum algorithm can exploit.

---
## Self-inverse property

$H^2 = I$, so applying $H$ twice returns the qubit to its original state. This also means the **adjoint is itself**: `Adjoint H = H` in Q#.

$$H \cdot H = \frac{1}{2}\begin{pmatrix}1 & 1\\1 & -1\end{pmatrix}\begin{pmatrix}1 & 1\\1 & -1\end{pmatrix} = \begin{pmatrix}1 & 0\\0 & 1\end{pmatrix}$$

This makes $H$ both unitary and Hermitian - a rare combination.

---
## Hadamard basis

$H$ defines a natural alternative orthonormal basis for a qubit:
...
- $+Z$ and $+X$ poles swap; $-Z$ and $-X$ poles swap
...
This single operation in $O(n)$ gates encodes all possible inputs - what would take $2^n$ steps classically. It is the starting point of [[Grover]], [[Deutsch-Jozsa]], [[Bernstein-Vazirani]], [[Simon]], and virtually every other quantum algorithm.

More generally, for any $n$-bit string $s$:

$$H^{\otimes n}|s\rangle = \frac{1}{\sqrt{2^n}}\sum_{x=0}^{2^n-1}(-1)^{s \cdot x}|x\rangle$$

where $s \cdot x = s_0 x_0 \oplus \cdots \oplus s_{n-1}x_{n-1}$ is the bitwise inner product. This is the **Walsh–Hadamard transform** - a real-valued QFT over $\mathbb{Z}_2^n$.

---
## Phase kickback

Phase kickback is the mechanism by which an oracle transfers phase info from an ancilla qubit back into the control register. It requires the ancilla to be in $|{-}\rangle$.

If an oracle marks a solution by flipping a target qubit ($X$ on target when condition met), and the target is $|{-}\rangle = H|1\rangle$:

$$\text{Oracle} \cdot |x\rangle|{-}\rangle = (-1)^{f(x)}|x\rangle|{-}\rangle$$

The $|{-}\rangle$ ancilla is unchanged; the phase $(-1)^{f(x)}$ is **kicked back** onto $|x\rangle$. Grover's oracle and QPE both rely entirely on this mechanism.
...
| [[Simon]] | $H^{\otimes n}$ before and after oracle; interference pattern reveals hidden period |
...
This means: **Hadamard conjugation swaps $X$ and $Z$ errors.** A $Z$ error on a qubit before $H$ is equivalent to an $X$ error after - important for error correction analysis.
