...
After encoding, the network processes the state via $1$ or more **variational layers**. Each layer can consist of:
...
- For **binary classification**, $1$ might measure a single qubit in the $Z$-basis, interpreting measurement outcomes or expectation values $⟨Z⟩$ as the network's prediction.
- For **multi-class** or **regression** tasks, measuring multiple [[Qubits]] or computing more complex observables yields higher-dimensional outputs.