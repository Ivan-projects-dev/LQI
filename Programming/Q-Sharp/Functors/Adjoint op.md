...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `adjoint auto;`       | Compiler reverses gate sequence & conjugates phases automatically. Works when body uses only adjointable ops.                                     |
| `adjoint self;`       | Operation is its own inverse (self-adjoint / Hermitian). Applies to $H, X, Y, Z$, [[CNOT]] - `Adjoint H == H`.                                    |
| `adjoint invert;`     | Same as `auto` - synonym used in older Q# syntax.                                                                                                 |
| `adjoint distribute;` | Used when generating `Controlled Adjoint`: distributes the adjoint into the controlled body. Needed when `controlled auto` alone is insufficient. |
