...
`Qubit` - represents single qubit. Cannot be copied, compared, or stored in classical data structures. Can only be passed to operations & funcs. Allocated via `use` or `borrow`. ([[Qubit management]])
