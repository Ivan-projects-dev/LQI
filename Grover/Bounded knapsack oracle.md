...
**Encoding**: represent each $q_i$ with $\lceil \log_2(b_i+1) \rceil$ [[Qubits]]. Total input register size: $\sum_i \lceil \log_2(b_i+1) \rceil$ [[Qubits]]. Each basis state $|x⟩ = |q_1 q_2 \cdots q_n⟩$ encodes one assignment.
