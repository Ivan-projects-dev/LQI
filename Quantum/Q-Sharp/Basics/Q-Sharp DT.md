...
`Int` - 64-bit signed integer. Literals: `42`, `-7`. Used for loop counters, qubit indices, integer encoding of bit strings.
`BigInt` - arbitrary-precision integer. Literal suffix `L`: `1000000000000L`. Used in Shor's algorithm for modular arithmetic.
...
`Range` - integer range. Literal: `0..n-1`, `0..2..10` (step 2), `n-1..-1..0` (reverse). Used in `for` loops & array slices.
...
Equivalent to `void`. Used as return type for operations with no output:
```csharp
...
**`IntAsBoolArray`, `ResultArrayAsIntBE` & conversion funcs**
