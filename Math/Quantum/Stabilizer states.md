...
**Example:** Consider, the Pauli operator $X⊗Z$. From the equations above, it is quite easy to see that $(X⊗Z)|+⟩⊗|0⟩=|+⟩⊗|0⟩$. [[Tensor]] product of the $+1$ eigenvectors of the individual operators gives $a +1$ eigenvector of the [[Tensor]]-product operator. $(X⊗Z)|−⟩⊗|1⟩=(−1)2|−⟩⊗|1⟩=|−⟩⊗|1⟩$. In this way, we can see that the operator $X⊗Z$ has two +1 eigenvectors (it also has two $− 1$ eigenvectors; why?).

Of course, any state that is a linear combo of $|+⟩⊗|0⟩$ and $|−⟩⊗|1⟩$ is also $a +1$ eigenstate of $X⊗Z$, $(X⊗Z)(α|+⟩⊗|0⟩+β|−⟩⊗|1⟩)=α(X⊗Z)|+⟩⊗|0⟩+β(X⊗Z)|−⟩⊗|1⟩$, $=α|+⟩⊗|0⟩+β|−⟩⊗|1⟩$.
Similarly, for any Pauli operator $P$ in $P_n$, we can construct states that are the $+1$ eigenvectors of the operator.

**Example:** Here is another example, $(X⊗I⊗X)|+⟩⊗|ϕ⟩⊗|+⟩=|+⟩⊗|ϕ⟩⊗|+⟩$,$(X⊗I⊗X)|−⟩⊗|ϕ⟩⊗|−⟩=|−⟩⊗|ϕ⟩⊗|−⟩$. $I$ is special because every state $|ϕ⟩$ is $a + 1$ eigenvector of $I$. Since $|ϕ⟩$ is arbitrary, we infer that $X⊗I⊗X$ stabilizes all states that are a linear combo of ${|+⟩⊗|0⟩⊗|+⟩,|−⟩⊗|0⟩⊗|−⟩,|+⟩⊗|1⟩⊗|+⟩,|−⟩⊗|1⟩⊗|−⟩}$.

**Example:** Consider, the operators $P_1=Z⊗Z⊗I$ and $P_2=Z⊗I⊗Z$, both in $P_3$.
- $Z⊗Z⊗I$ stabilizes $4$ orthogonal states ${|000⟩,|001⟩,|110⟩,|111⟩}$.
- $Z⊗I⊗Z$ stabilizes $4$ orthogonal states ${|000⟩,|010⟩,|101⟩,|111⟩}$.
The intersection of these sets is ${|000⟩,|111⟩}$. This means that any state that is a linear combo of these $2$ states will be stabilized by both $P_1$ and $P_2$.
