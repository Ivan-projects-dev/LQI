...
**Quantum Development Kit (QDK)** - open-source SDK supporting Q#, Qiskit, Cirq, & [[OpenQASM]]. Provides language extensions for VS Code, Jupyter notebook integration, domain libraries for chemistry & ML, & a built-in sparse-state simulator. QDK compiles programs to **QIR (Quantum Intermediate Representation)** - a common LLVM-based IR that lets any supported language target any provider backend.

**Azure Quantum Resource Estimator ([[QRE]])** - estimates the number of physical & logical [[Qubits]], runtime, & T-factory counts required to run an algorithm at fault-tolerant scale. Supports configurable qubit parameter sets (superconducting, trapped-ion), two built-in QEC schemes ([[Surface Code]], floquet code), & custom error correction models. Computes space-time tradeoff frontiers to balance qubit count vs. runtime. Completely **free** - no Azure account required; runs in VS Code or at quantum.microsoft.com.
