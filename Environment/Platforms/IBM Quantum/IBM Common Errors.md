...
1. `AttributeError: 'QiskitRuntimeService' object has no attribute 'run'` 
...
```
2. `IBMInputValueError: 'The instruction ... is not supported'`
...
```
3. `KeyError: 'meas'` when extracting counts
...
```
4. Circuit runs but all results are `00000`
...
5. `QiskitRuntimeError: No` [[IBM Quantum]] `account found`
...
6. `qc.draw('mpl')` crashes with `ModuleNotFoundError`
