...
where $|j\rangle$ denotes the $j$th computational basis state of $n$ [[Qubits]] & $\|\vec{x}\| = \sqrt{\sum_k |x_k|^2}$ is the normalization.
...
for some int $p$, where
$$\bar{X} = (x_{11}, \ldots, x_{1N},\; x_{21}, \ldots, x_{2N},\; \ldots,\; x_{M1}, \ldots, x_{MN}) \in \mathbb{R}^{MN}$$
is the concatenation of all data points & $C_D$ is the normalization constant. The constraint is $2^p \geq MN$, i.e. $p \geq \log_2(MN)$.
...
Amplitude encoding is memory-efficient in qubit count, but the [[State preparation]] cost can dominate & must be included when evaluating any claimed quantum speedup.
