...
**`M(q)` - Z-basis measurement**

Measures qubit `q` in the Pauli-Z basis. Returns `Result`. The qubit collapses to $|0\rangle$ (`Zero`) or $|1\rangle$ (`One`).
```csharp
let result = M(q);
if result == One {
    Message("Measured |1⟩");
...
**`Measure(bases, qs)` - multi-qubit Pauli measurement**

Measures array of [[Qubits]] jointly in specified Pauli bases. Returns a single `Result` representing the parity of all individual outcomes.
```csharp
...
**`MResetZ(q)` - measure & reset**

Measures in Z-basis & immediately resets the qubit to $|0\rangle$. Most concise way to end a computation:
```csharp
...
Measure in $X$ or $Y$ basis & reset. Equivalent to applying the appropriate basis-change gate before measuring:
