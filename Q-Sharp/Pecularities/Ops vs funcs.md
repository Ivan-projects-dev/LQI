#Q-Sharp
Q# distinguishes $2$ kinds of callables: **ops** & **funcs**. The distinction is fundamental - it determines what the callable can do, how it can be composed, & whether it can be inverted or controlled.

**Operation** is a quantum subroutine - it can allocate [[Qubits]], apply gates, perform measurements, & call other ops. ops may have side effects on [[Quantum state]].
```csharp
...
Return type `Unit` means no classical output. ops can return classical values too:
```csharp
...
funcs use `->` in their type signature (vs `=>` for ops):
```csharp
let f : Int -> Double = IntAsDouble; // func type
let op : Qubit => Unit is Adj = H; // operation type
```
funcs **cannot** call ops. ops **can** call funcs.
...
| Call from op             | Yes                  | Yes         |

Both ops & funcs are first-class values & can be passed as arguments:
```csharp
// Pass op as argument
...
```
Used in [[Grover search]] to define inline oracles without declaring separate named ops.
